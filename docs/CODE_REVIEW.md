...
| 10 | gpu | Kein `cudaStream_t`-Parameter — blockiert Multi-GPU-Concurrency auf the HPC cluster H100s | ✅ Resolved: `OverlapKernelLaunchConfig::stream` hinzugefuegt (overlap_kernel.cuh L59) |
...
- ✅ Resolved: Hardcoded Username `<user>` in Snakefile — jetzt via `os.environ.get("USER", "<user>")` parametrisiert (Snakefile L15); Fallback-String bleibt, aktiver Pfad nutzt $USER.
...
Vor empirischem Baseline-Run (1 Genom, hifiasm auf the HPC cluster) fixen:
...
2. **PR 2**: Snakefile `/tmp`-Bug + `SHARED`-Parametrisierung → the HPC cluster-ready — ✅ Resolved
3. **PR 3**: `.github/workflows/ci.yml` mit ASan/TSan → CI aktiv — ✅ Resolved
4. **Phase 0 Baseline-Run** auf the HPC cluster — ✅ Resolved (HG002 SLURM 1823405 durchgelaufen 2026-04-17; 8 HPRC-Genome nachgelaufen)
