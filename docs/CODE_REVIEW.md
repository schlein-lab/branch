...
| 10 | gpu | Kein `cudaStream_t`-Parameter — blockiert Multi-GPU-Concurrency auf the HPC cluster H100s |
...
- Hardcoded Username `<user>` in Snakefile (`SHARED`-Pfad)
...
2. **PR 2**: Snakefile `/tmp`-Bug + `SHARED`-Parametrisierung → the HPC cluster-ready
3. **PR 3**: `.github/workflows/ci.yml` mit ASan/TSan → CI aktiv
4. **Phase 0 Baseline-Run** auf the HPC cluster
