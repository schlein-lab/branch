...
| Hardcoded `<user>` | MEDIUM | `SHARED` path hardcoded, should use `{USER}` |
| `mem_mb = 0` | LOW | Works (the HPC cluster ignores), but unclear intent |
...
| `TMPDIR=/tmp` | OK | Correct per the HPC cluster docs |
