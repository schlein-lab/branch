...
## Workflow Issues (the HPC cluster)
...
| Hardcoded `<user>` | MEDIUM | `SHARED` path hardcoded, should use `{USER}` | ✅ Resolved: `USER = os.environ.get("USER", "<user>")` (Snakefile L15), `SHARED`/`BEEGFS_BASE` nutzen `{USER}` |
| `mem_mb = 0` | LOW | Works (the HPC cluster ignores), but unclear intent | ⬜ Open: weiterhin `mem_mb = 0` in Regeln, keine Klaerung im Kommentar |
...
| `TMPDIR=/tmp` | OK | Correct per the HPC cluster docs | ✅ Resolved (already OK) |
