...
## Running Phase 0 on the HPC cluster

The Schlein-lab Phase-0 driver is a Snakemake workflow tailored to the the HPC cluster cluster (RRZ Hamburg). It expects BeeGFS storage (`/beegfs/u/$USER/<group>/<lab>/$USER`), Apptainer for tool containers, and the SLURM executor plugin. From a the HPC cluster login node:
...
# submit the workflow
./phase0/submit_slurm.sh                       # HG002 default
./phase0/submit_slurm.sh --config sample=NA24385
./phase0/submit_slurm.sh --dry-run             # show DAG without submitting
```

`submit_slurm.sh` enforces the cluster's quirks for you: no `--mem` (RRZ rejects it), `TMPDIR=/tmp` (home is read-only inside batch jobs), `--bind /beegfs` for Apptainer, the SLURM executor profile under `workflow/config/slurm/`, and a 50-job concurrent cap. Off-cluster, the same Snakefile runs via `snakemake -s workflow/Snakefile` once you point `branch_bin`, `hifi_dir`, and the reference paths at your own filesystem — the pipeline itself is filesystem-agnostic.
...
- **`phase0/`** — the HPC cluster submit helper plus Phase-0 task done-files.
